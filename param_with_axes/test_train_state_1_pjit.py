...
parser.add_argument('--cts', action='store_true', help='customized_train_state')
args = parser.parse_args()

use_param_with_axes = args.axes
use_customized_train_state = args.cts
print("DEBUG: param_with_axes", use_param_with_axes)
print("DEBUG: customized_train_state", use_customized_train_state)
...
class CustomizedTrainState(struct.PyTreeNode):
  """
  Args:
...
TrainState = CustomizedTrainState if use_customized_train_state else train_state.TrainState
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    if use_param_with_axes:
      input_amax_history = param_with_axes(
          'input_amax_history_fp8_meta', *amax_history_args, axes=('fp8_params',))
      #input_amax_history = jnp.asarray(input_amax_history, jnp.float32)
      kernel_amax_history = param_with_axes(
          'kernel_amax_history_fp8_meta',
          *amax_history_args, axes=('fp8_params',))
      #kernel_amax_history = jnp.asarray(kernel_amax_history, jnp.float32)
      output_grad_amax_history = param_with_axes(
          'output_grad_amax_history_fp8_meta',
          *amax_history_args, axes=('fp8_params',))
      #output_grad_amax_history = jnp.asarray(output_grad_amax_history, jnp.float32)

      input_scale = param_with_axes(
          'input_scale_fp8_meta', *scale_args, axes=('fp8_params',))
      #input_scale = jnp.asarray(input_scale, jnp.float32)
      kernel_scale = param_with_axes(
          'kernel_scale_fp8_meta', *scale_args, axes=('fp8_params',))
      #kernel_scale = jnp.asarray(kernel_scale, jnp.float32)
      output_grad_scale = param_with_axes(
          'output_grad_scale_fp8_meta', *scale_args, axes=('fp8_params',))
      #output_grad_scale = jnp.asarray(output_grad_scale, jnp.float32)
    else: 
...
def train_loop(state, x, dy):
  def loss_fn(params, x, dy):
  ...
  state_var = state.variables() if use_customized_train_state else state.params
  grad_fn = jax.value_and_grad(loss_fn, argnums=[0, 1])
  loss, grads = grad_fn(state_var, x, dy)
  new_state = state.apply_gradients(grads=grads[0])
...
def run_me():
  ...
  if use_customized_train_state:
    ts_args['model_variables'] = initialized_variables
  else:
    ts_args['params'] = initialized_variables
  state = TrainState.create(**ts_args)

  iters = 200
  pjit_train_fn = pjit(train_loop)
  with mesh:
    # warmup
    for _ in range(100):
      state, loss, grads = pjit_train_fn(state, x_data, dy_data)
    ...
    grads[1].block_until_ready()
    end_time = time.time()
    duration_in_milliseconds = (end_time - start_time) * 1000
    print("XXX duration (ms):", duration_in_milliseconds / iters)
